...
\subsection{Result}
We run our experiments on Dumbo in NYU. For the comparison with other related works, we plan to migrate our setting on AWS since we have the authentication to install other packages on AWS. On Dumbo, we won't have the right to do that.
...
\hline
Standard Scale   & 1 m 51 s     & 48.36 s & 41.58 s            \\ 
Min-max Scale  &  1 m 50 s   & 48.74 s  & 41.69 s             \\  
Max-abs Scale  & 1 m 50 s     &  47.27 s  & 41.52 s             \\ 
Normalization           &  1.8 s     &  1.2 s & 1.1 s             \\ 
